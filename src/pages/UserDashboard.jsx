...
function UserDashboard() {
  ...
  const { user, token } = useContext(AuthContext); // ✅ token added
    ...
    if (window.confirm("Are you sure you want to delete this blog?")) {
      try {
        await API.delete(`/posts/${id}`, {
          headers: { Authorization: `Bearer ${token}` } // ✅ Secure delete
        });
        ...
        {blogs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {blogs.map((blog, index) => (
              <div
                key={blog._id}
                className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 p-5 flex flex-col"
              >
